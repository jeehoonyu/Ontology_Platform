...
try {
    ...
    Write-Output "PRODUCTION_ACCEPTANCE_PASSED: OIDC, RBAC, tenant boundary, 50-reader load, cross-replica collaboration, serialized migration startup, API restart, and backup/restore recovery verified."
} finally {
    if (-not $KeepStack) {
