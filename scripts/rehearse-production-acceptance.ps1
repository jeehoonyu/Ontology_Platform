...
try {
    ...
    Write-Output "PRODUCTION_ACCEPTANCE_PASSED: OIDC, RBAC, project-owned data onboarding, cross-project denial, 50-reader load, cross-replica collaboration, job idempotency, abandoned-worker chaos recovery, serialized migration startup, API restart, and backup/restore recovery verified."
} finally {
    if (-not $KeepStack) {
