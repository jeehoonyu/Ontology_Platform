...
const concurrency = Number(process.env.OIDC_SCALE_LOGIN_CONCURRENCY || "20");
...
async function authenticate(browser: Browser, index: number): Promise<IdentityResult> {
...
test("200 distinct OIDC identities authenticate across both replicas", async ({ browser }) => {
  ...
  const workers = Array.from({ length: Math.min(concurrency, userCount) }, async () => {
    while (next <= userCount) {
      const index = next++;
      results.push(await authenticate(browser, index));
    }
  });
