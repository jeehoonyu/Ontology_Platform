...
import {
  ...
  type DecisionScorecard, type EntityCandidate, type EntityJob, type ObjectSnapshot
...
export function DecisionWorkspace() {
  ...
  // The whole job, not only its candidates: how much of the type it compared is on it.
  const [entityJob, setEntityJob] = useState<EntityJob | null>(null);
  ...
  const selectType = (nextId: string) => void act("Ontology context loaded", async () => {
    setObjectTypeId(nextId); setEvaluation(null); setObjectId(""); setExplanation(null); setTimeline([]); setCandidates([]); setEntityJob(null); setScenario(null);
  ...
  return <Page title="Decision Intelligence" subtitle="Explainable risk, temporal evidence, entity resolution, scenarios, and governed agent plans">
    ...
    {tab === "entity" ? <EntityPanel job={entityJob} candidates={candidates} busy={busy} onRun={(resolutionFields, threshold) => void act("Entity review queue generated", async () => { const job = await createEntityJob(projectId, objectTypeId, resolutionFields, threshold); setEntityJob(job); setCandidates(job.candidates || []); })} onAccept={(id) => void act("Candidate accepted", () => updateCandidate(id, true))} onReject={(id) => void act("Candidate rejected", () => updateCandidate(id, false))} /> : null}
...
// "clear" only when the latest entity-resolution job compared this object with every other and
// left no pending candidate. A job compares every pair among the first N objects by id and pairs
// the rest only where they share an exact value, so an object past the scan that shares none reads
// "no exact-match candidate" -- unless the job left a crowded value unpaired, when nothing narrower
// than "not compared" is true. An object made after the job was not read at all.
function duplicateStatus(explanation: DecisionExplanation | null): string {
  if (!explanation) return "not loaded";
  const warnings = explanation.duplicate_warning_count ?? explanation.duplicate_warnings.length;
  if (warnings) return `${warnings.toLocaleString()} warnings`;
  const coverage = explanation.duplicate_coverage;
  if (coverage?.compared) return "clear";
  if (coverage?.exact_checked && !coverage.exact_values_skipped) return "no exact-match candidate";
  return "not compared";
}
...
function EntityPanel({ job, candidates, busy, onRun, onAccept, onReject }: { job: EntityJob | null; candidates: EntityCandidate[]; busy: string; onRun: (fields: string[], threshold: number) => void; onAccept: (id: string) => void; onReject: (id: string) => void }) {
  const [fieldText, setFieldText] = useState("name, serial_number"); const [threshold, setThreshold] = useState(70);
  // The job compared every pair among the first `scanned` objects by id. When that is not
  // every object, the queue says so, and so does an empty queue: "No candidates" claimed
  // the whole type.
  const scanned = job?.objects_scanned ?? null; const inScope = job?.objects_in_scope ?? null;
  const partial = scanned !== null && inScope !== null && scanned < inScope;
  // The exact pass pairs every object the type held on a shared value of a matching field, so the
  // objects past the scan were compared where they share one. A job from before the pass has none.
  const exact = job?.exact_objects ?? null; const skipped = job?.exact_values_skipped ?? 0; const ceiling = job?.exact_group_ceiling ?? 0;
  const fieldNames = (job?.fields || []).join(" or "); const rest = (inScope ?? 0) - (scanned ?? 0);
  const crowded = skipped ? ` ${skipped.toLocaleString()} shared ${skipped === 1 ? `value held by more than ${ceiling.toLocaleString()} objects was` : `values, each held by more than ${ceiling.toLocaleString()} objects, were`} not paired.` : "";
  const emptyTitle = exact === null ? `No candidates among the first ${(scanned ?? 0).toLocaleString()} objects`
    : `No candidates among the first ${(scanned ?? 0).toLocaleString()} objects, or from an exact ${fieldNames} match across all ${exact.toLocaleString()}`;
  return <div className="decision-evidence-layout"><Panel title="Resolution Settings"><label className="stacked-field"><span>Matching fields</span><input aria-label="Entity resolution fields" value={fieldText} onChange={(event) => setFieldText(event.target.value)} /></label><label className="stacked-field"><span>Minimum confidence: {threshold}%</span><input aria-label="Entity resolution threshold" type="range" min="40" max="100" value={threshold} onChange={(event) => setThreshold(Number(event.target.value))} /></label><button className="primary" disabled={!!busy || !splitFields(fieldText).length} onClick={() => onRun(splitFields(fieldText), threshold)}><GitCompareArrows size={15} />Find duplicates</button></Panel><Panel title="Candidate Review Queue">{partial ? <p className="table-truncated" role="note">{exact === null ? <>Compared the first {(scanned ?? 0).toLocaleString()} of {(inScope ?? 0).toLocaleString()} objects, by id. Pairs involving the other {rest.toLocaleString()} were not compared.</> : <>Compared every pair among the first {(scanned ?? 0).toLocaleString()} of {(inScope ?? 0).toLocaleString()} objects, by id, and paired all {exact.toLocaleString()} on an exact {fieldNames}, ignoring case and punctuation. Pairs involving the other {rest.toLocaleString()} were compared only where they share such a value.{crowded}</>}</p> : null}<div className="decision-candidate-list">{candidates.map((candidate) => <article key={candidate.id}><header><span><strong>{candidate.object_ids.join(" + ")}</strong><small>{candidate.reasons.length} matching signals</small></span><StatusBadge value={`${candidate.score}%`} /></header><div className="button-row"><button disabled={candidate.status !== "PENDING" || !!busy} onClick={() => onAccept(candidate.id)}><Check size={14} />Accept merge</button><button disabled={candidate.status !== "PENDING" || !!busy} onClick={() => onReject(candidate.id)}><X size={14} />Reject</button><StatusBadge value={candidate.status} /></div></article>)}</div>{!candidates.length ? <EmptyState title={partial ? emptyTitle : "No candidates"} description="Choose matching fields and generate a deterministic review queue." /> : null}</Panel></div>;
}
