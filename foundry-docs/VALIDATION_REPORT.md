...
- Production acceptance: `.github/workflows/ci.yml` automatically gates pull requests with 114 backend scripts, docs conformance, dependency audit, SQLite/PostgreSQL migrations, responsive WCAG browser acceptance, Compose validation, and a production image build. `scripts/rehearse-production-acceptance.ps1` remains the final release rehearsal: it deploys digest-pinned Keycloak/Postgres fixtures with two API replicas, validates real OIDC claims and backend RBAC, exercises 50 authenticated readers, cross-replica collaboration, worker recovery, API restart, and fresh-volume backup/restore.
...
| ModelOps | High local analog | Project-owned objectives, training, eval gates/data/subsets, releases, deployments/configs, adapters, inference logs, drift monitors/runs, RBAC, migration, and snapshot recovery |
