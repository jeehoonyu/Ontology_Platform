...
This repository implements a local deterministic analog of public Palantir Foundry/AIP concepts. It does not copy Palantir code, claim proprietary API compatibility, or reproduce managed Foundry infrastructure. Validation is based on public docs, the public Palantir Foundry documentation, and executable behavior tests.
...
- Runtime operations: durable jobs now emit project-scoped correlated observations for queue, claim, heartbeat, retry, recovery, success, failure, and cancellation. Request-hashed, database-unique receipts prevent bounded-history and cross-replica duplicate enqueue while rejecting changed requests behind reused keys. Stale-job reapers use row locking, emit dedicated recovery spans/audit/Ops evidence, and fence abandoned lease tokens. Rolling budgets gate admission, SLO evaluations emit breach events, and the Control Panel exposes human-readable latency, availability, usage, and cost evidence. Independently deployable worker daemons use hashed, project-scoped service tokens and add capability scope, concurrent polling, health endpoints, graceful drain, fair project queues, atomic lease contention handling, and stale-worker fencing. A credential-free production availability observer persists a migration-scoped hash-chained journal and counts every missed slot as unavailable. The complete pilot scheduler requires a distinct restored target, bearer-authenticated outbox markers, matching database/runtime migration heads, hash-chained RPO/RTO journals, and failure-preserving unattended recovery runs. The shipped PostgreSQL reference driver checksums database/snapshot/plugin artifacts, authenticates their manifest with a separate HMAC key, and restores them into fresh volumes in a separate Compose project; a real two-project rehearsal recovered the source marker and accepted an authenticated write in 19.094 seconds. This is implementation evidence, not a substitute for the still-unclaimed seven-day and repeated-recovery acceptance windows.
- OntologyOS runtime core: normalized semantic definitions, append-only object changes, typed temporal/GIS object queries, bounded graph queries, immutable Parquet/JSONL snapshots, compiled pipeline plans, real DuckDB/Arrow snapshot previews, a provider-neutral governed model gateway, and versioned durable agent tasks are available through additive `/api/v1` contracts. Published Pipeline, Workshop, and visual-artifact consumer versions bind immutably to exact production ontology revisions, checksums, and referenced properties. Dynamic health distinguishes current, compatible-stale, and broken consumers; publish, rollback, property-aware impact analysis, and the React Ontology release workflow expose that evidence directly. Breaking publication requires explicit human acknowledgement of the named consumers and migration plan, and the backend rejects any missing or stale binding-ID set after recomputing current impact.
...
- Pipeline scale: bounded local or S3-compatible Parquet registration avoids row hydration, recovers Hive fields, and records immutable per-file manifests; S3 snapshots use an integrity-checked, quota-bounded, lease-aware execution cache with operational metrics and coordinated concurrent downloads; compiled DuckDB plans use order-independent named parameters and SQL-native branches, reshape, validation, window, point/radius, MGRS, high-vertex polygon-geofence, and spatial-join operations; atomic Hive-partitioned delivery records every source snapshot and is idempotent by execution job. DuckDB jobs execute inside independently deployable worker processes rather than the API. A production-image PostgreSQL/MinIO/Toxiproxy rehearsal loads the image into two independent Docker daemons, kills the first worker with exit 137 after it caches 3 of 32 partitions, lets its lease expire, recovers attempt two in the replacement daemon with a separate image store, filesystem, and cache, and publishes exactly one fenced 10,000-row output. The strict local profile processed eight files/10M fact rows through a selective 9,218-position polygon with a hole, stable IDs, radius GIS, MGRS, a dimension join, and a partitioned window with preview p95 1,459.595 ms, delivery 1,515.835 ms, 20 output partitions, and only 20 Python result rows. A second plan joins 10M by 10M rows, unions 20M rows, and pivots 512 columns. The real-MinIO reference independently verifies 40 ms downstream latency with 5 ms jitter: one-million-row registration took 884.284 ms, cold query 534.057 ms, warm query p95 172.907 ms, cold pipeline 502.387 ms, and warm pipeline p95 97.444 ms. Four concurrent cold-cache workers completed in 540.532 ms with exactly eight downloads, 24 coordinated cache hits, and zero integrity failures. Both recovery daemons still ran on one physical host; true multi-host execution, intended-provider networks, and distributed partition scheduling remain open.
- Production acceptance: `.github/workflows/ci.yml` automatically gates pull requests with the current backend scripts, 72-row docs conformance matrix, dependency audit, SQLite/PostgreSQL migrations through `0042_stream_outer_joins`, digest-pinned plugin OCI and governed-egress rehearsals, isolated executor image builds, ontology query and mixed-workload plans, a partitioned 1M-row advanced snapshot pipeline smoke benchmark, a latency-injected four-worker MinIO smoke benchmark, a real PostgreSQL worker-process-loss recovery rehearsal with independent caches, two-replica collaboration scale and WebSocket chaos rehearsals, interrupted inner/outer stream transaction recovery, responsive WCAG browser acceptance, Compose validation, and production image builds. `scripts/rehearse-production-acceptance.ps1` remains the final release rehearsal: it deploys digest-pinned Keycloak/Postgres fixtures with two API replicas, validates real OIDC claims, browser WebSockets, and backend RBAC, provisions 200 distinct scale identities, exercises 50 authenticated readers, cross-replica collaboration, real production-image worker loss across both ordinary containers and independent Docker daemons with one fenced publication, API restart, and fresh-volume backup/restore.
...
- Evidence: the public [Foundry documentation](https://www.palantir.com/docs/foundry/), `docs/GOAL_ACCEPTANCE_2026-07-28.md`, `oms/test_docs_conformance.py`, and focused production/runtime tests.
...
| Connectors and streaming | High local analog | Live REST/PostgreSQL/S3-compatible/SFTP/Kafka adapters, encrypted credentials, SigV4, pinned SSH hosts, TLS/SASL, SSRF/read-only protections, durable cursors, event-time watermarks/windows, quarantine, backpressure, exactly-once processor receipts, inner/left/right/full interval joins, idle-key watermark advancement, immutable unmatched receipts, bounded join-state compaction, budgets, dead letters, and recovery evidence |
...
1. Extend the achieved strict 10-million-object/50-million-link mixed, fresh-volume backup, streaming-failover, and dependency-gated partition-worker profiles with longer-duration saturation, true multi-host workers, and repeated scheduled recovery evidence; add projections or further partitioning only where measurements require them.
