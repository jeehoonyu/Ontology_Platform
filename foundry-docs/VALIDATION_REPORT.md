...
- Frontend direction: a React/Vite/TypeScript shell now serves the core evaluator workspaces when built, while the legacy static UI remains as a migration fallback.
...
| Frontend product foundation | Medium-high | React/Vite core evaluator shell with legacy fallback |
...
- UI workspaces are Foundry-style local approximations, not copied Foundry frontends. The React shell currently covers the evaluator path first; legacy workspaces remain during migration.
