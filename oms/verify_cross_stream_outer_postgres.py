"""Rehearse concurrent outer-join finalization on PostgreSQL."""
...
if not os.environ.get("DATABASE_URL", "").startswith("postgresql"):
    raise SystemExit("verify_cross_stream_outer_postgres.py requires a PostgreSQL DATABASE_URL")
...
from app.stream_processing import StreamJoinOuterReceipt, StreamProcessingReceipt  # noqa: E402


prefix = f"pg_outer_{uuid.uuid4().hex[:8]}"
...
with TestClient(app) as client:
    checked(client.post("/data-assets", json={
        "id": output_id, "project_id": "default", "display_name": "PostgreSQL outer output",
    ...
    checked(client.post("/api/v1/streams/processors", json={
        ...
        "join_stream_id": right_stream, "display_name": "PostgreSQL full outer interval join",
        "timestamp_field": "event_ts", "join_left_key": "asset_id",
        "join_right_key": "asset_id", "join_time_tolerance_seconds": 5,
        "join_type": "full", "late_policy": "quarantine", "target_asset_id": output_id,
    ...
    processed = checked(client.post(f"/api/v1/streams/processors/{processor_id}/process", json={}))
    assert processed["records_processed"] == 50 and processed["outer_joins_emitted"] == 0


def advance(worker_number):
    with TestClient(app) as client:
        return checked(client.post(
            f"/api/v1/streams/processors/{processor_id}/watermarks",
            json={"side": "right", "join_key": "shared-key", "watermark": 1100},
            headers={"X-Actor": f"postgres-outer-worker-{worker_number}"},
        ))


with ThreadPoolExecutor(max_workers=2) as pool:
    results = list(pool.map(advance, range(2)))
assert sorted(row["outer_joins_emitted"] for row in results) == [0, 50], results
...
with TestClient(app) as client:
    ...
    late = checked(client.post(f"/api/v1/streams/processors/{processor_id}/process", json={}))
    assert late["records_late"] == 50 and late["records_quarantined"] == 50
    assert checked(client.post(
        f"/api/v1/streams/processors/{processor_id}/watermarks",
        json={"side": "right", "join_key": "shared-key", "watermark": 1100},
    ))["outer_joins_emitted"] == 0

with SessionLocal() as db:
    assert db.query(StreamJoinOuterReceipt).filter(
        StreamJoinOuterReceipt.processor_id == processor_id,
    ).count() == 50
    assert db.query(StreamProcessingReceipt).filter(
        StreamProcessingReceipt.processor_id == processor_id,
    ).count() == 100
    assert len(db.get(DataAsset, output_id).records) == 50

indexes = {row["name"] for row in inspect(engine).get_indexes("stream_join_outer_receipts")}
assert "ix_stream_join_outer_receipts_finalized" in indexes

print("PostgreSQL concurrent outer-join finalization rehearsal passed.")
engine.dispose()
