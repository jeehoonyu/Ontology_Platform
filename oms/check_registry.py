...
DECLARATIONS: Dict[str, Dict[str, str]] = {
    ...
    "benchmark_ontology_mixed_workload_postgres": {"gates": "Tier B mixed workload", "cadence": "per release candidate"},
    "benchmark_pipeline_scale": {"gates": "Tier B pipeline scale", "cadence": "smoke every push, reference per release candidate"},
    "verify_artifact_review_postgres": {"gates": "artifact review survives a real database", "cadence": "per release candidate"},
    "verify_collaboration_scale_postgres": {"gates": "Tier B collaboration", "cadence": "per release candidate"},
    "verify_collaboration_websocket_chaos_postgres": {"gates": "Tier B chaos, websocket process loss", "cadence": "per release candidate"},
    "verify_cross_stream_join_postgres": {"gates": "cross-stream joins on a real database", "cadence": "per release candidate"},
    "verify_cross_stream_outer_postgres": {"gates": "cross-stream outer joins", "cadence": "per release candidate"},
    "verify_cross_stream_partition_postgres": {"gates": "Tier B chaos, partition recovery", "cadence": "per release candidate"},
    "verify_cross_stream_outer_partition_postgres": {"gates": "Tier B chaos, outer-join partition recovery", "cadence": "per release candidate"},
    "verify_event_stream_routing_postgres": {"gates": "durable event-to-stream routing", "cadence": "per release candidate"},
    "verify_ontology_query_postgres": {"gates": "the ontology query compiler on PostgreSQL", "cadence": "per release candidate"},
    "verify_stream_processing_postgres": {"gates": "stream ordering and processor fencing", "cadence": "per release candidate"},

    # --- need a database and Docker ------------------------------------------
    "rehearse_ontology_scale_backup_restore": {"gates": "Tier B durability, fresh-volume restore", "cadence": "per release candidate"},
    "rehearse_ontology_scale_replica_failover": {"gates": "Tier B durability, replica promotion", "cadence": "per release candidate"},
    "rehearse_ontology_scale_recovery": {"gates": "process-restart recovery of the reference fixture", "cadence": "per release candidate"},
    "rehearse_pipeline_worker_recovery": {"gates": "pipeline worker loss and lease fencing", "cadence": "per release candidate"},

    # --- need a broker, an object store, an OCI sandbox, or a remote service --
    "rehearse_event_kafka": {"gates": "outbox mirroring into Kafka", "cadence": "before enabling downstream consumers"},
    "rehearse_event_kafka_recovery": {"gates": "broker interruption and recovery", "cadence": "before enabling downstream consumers"},
    "rehearse_kafka_connector": {"gates": "the durable Kafka connector path", "cadence": "before enabling downstream consumers"},
    "benchmark_object_storage_minio": {"gates": "object-storage snapshot throughput", "cadence": "per release candidate"},
    "rehearse_s3_snapshot_minio": {"gates": "S3-backed snapshot round trip", "cadence": "per release candidate"},
    "rehearse_plugin_egress": {"gates": "signed-plugin egress denial and CA pinning", "cadence": "per release candidate"},
    "rehearse_plugin_oci": {"gates": "the hardened plugin OCI boundary", "cadence": "per release candidate"},
    "rehearse_sftp_connector": {"gates": "the durable connector path against real SFTP", "cadence": "when the connector changes"},
}
