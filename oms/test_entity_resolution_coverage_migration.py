"""Entity-resolution job coverage columns upgrade and downgrade from the prior head."""
...
COLUMNS = {"objects_in_scope", "objects_scanned", "last_scanned_id"}

with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as temporary:
    database_url = f"sqlite:///{os.path.join(temporary, 'entity-coverage-migration.db')}"
    ...
    alembic("upgrade", "0043_drop_orphaned_value_types")
    engine = create_engine(database_url)
    with engine.begin() as connection:
        # 0001 builds tables from today's models, so a fresh chain already has the columns.
        # Remove them to stand for a database that reached 0043 before they existed.
        ...
        assert not COLUMNS & job_columns(connection), "the prior head still has coverage columns"
        connection.execute(text(
            ...
            "VALUES ('before-coverage', 'default', 'asset', '[]', 'COMPLETED', 1, 2, 0)"
        ))
    ...
    with engine.connect() as connection:
        ...
        row = connection.execute(text(
            "SELECT objects_in_scope, objects_scanned, last_scanned_id FROM entity_resolution_jobs WHERE id = 'before-coverage'"
        )).one()
        # A job that ran before the columns existed says nothing it cannot know.
        assert tuple(row) == (None, None, None), row
        assert connection.execute(text("SELECT version_num FROM alembic_version")).scalar_one() == current_head()

    # Applied twice, as CI does.
    alembic("upgrade", "head")

    alembic("downgrade", "0043_drop_orphaned_value_types")
    with engine.connect() as connection:
...
print("Entity resolution coverage migration verified.")
