...
def usage_summary(project: Optional[str] = Query(default=None), metric: Optional[str] = Query(default=None),
                  group_by: str = Query(default="project"), db: Session = Depends(get_db)):
...
def create_quota(body: QuotaCreate, db: Session = Depends(get_db)):
    qid = uuid.uuid4().hex
...
def list_quotas(db: Session = Depends(get_db)):
    return [{"scope_type": q.scope_type, "scope_id": q.scope_id, "metric": q.metric, "limit_value": q.limit_value}
            for q in db.query(UsageQuota).all()]


@router.post("/admin/usage/check-quota")
def check_quota(body: QuotaCheckRequest, db: Session = Depends(get_db)):
