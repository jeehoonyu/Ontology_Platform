...
def import_project(
    ...
    for row in snapshot.get("entity_resolution_jobs") or []:
        row.setdefault("project_id", "default")
        row.setdefault("created_at", now)
        track(_upsert_model(db, decision_intelligence.EntityResolutionJob, row, ["id", "project_id", "object_type_id", "fields", "status", "created_at", "completed_at", "candidate_count", "objects_in_scope", "objects_scanned", "last_scanned_id", "exact_objects", "exact_values_skipped"]))
    for row in snapshot.get("entity_candidates") or []:
        row.setdefault("project_id", "default")
        row.setdefault("created_at", now)
