...
| 2026-08-09 | Could the durability gate show which schema it measured? | No. Both rehearsals stamped `current_head()`, which reads the repository's migration files and says what the *code* declares. Both scripts already read `alembic_version` from the database and neither passed it on, in the gate whose subject is whether a schema survives a restore | `record()` requires a verified `observed_head` |
| 2026-08-09 | So the 2026-08-08 evidence was false? | **No, and asserting it was is the error of this session.** Finding the default `ontology_scale_reference` volume at `0031` and seeing both harnesses default to it, I reported misattribution as fact without opening the harness evidence -- which records `source_container: ontology_postgres` at `0041` on both source and restored target. The runs were correct. An inference was published as a finding | Retracted in code, docs and the ledger |
| 2026-08-09 | Then was the fix worth making? | Yes, and the near miss is the argument. Those runs avoided the stale volume only because an operator passed overrides; the default path stamps today's head onto a nine-day-old schema and the file cannot show it. Both rows were re-run rather than trusted, because "the file happens to be accurate" is not a property the gate can check | 2 rows excluded, both re-measured at 0042 |
| 2026-08-13 | Could anyone else actually run this repository? | No. A fresh clone with a venv built from the pinned manifest could not import the test suite: `httpx` is required by `starlette.testclient` and was undeclared, present here only because an unrelated project installed it into the shared global interpreter. Pinning the declarations also failed to determine the closure -- 11 of 42 transitive packages resolved differently, starlette 1.2.1 against 1.6.0 among them, which is what broke the import | `requirements.lock` pins all 45; digest reproduces |
| 2026-08-13 | What reproduces across environments, and what does not? | Structure reproduces exactly and timing does not. `pipeline_scale` re-run in the locked environment matched every deterministic measurement to the digit -- 10M rows, 20 partitions, 9,218 geofence positions -- and drifted up to **+33%** on latency, on the same machine with a byte-identical closure. A threshold within a third of a measured value cannot distinguish a regression from the same run twice | Thresholds belong against the spread, not one reading |
| 2026-08-13 | Did the first reproduction succeed? | No, and it was reported here as succeeding. The run crashed at import, the clone's evidence file was never rewritten, and the comparison was the committed file against itself -- which agreed perfectly, because a silent no-op always does. The check is not "do the numbers match" but "did the file change, and then match" | `git status` before any comparison |
| 2026-08-13 | What third-party code produced every number in this repository? | Unrecorded, and unpinnable as written. All sixteen dependencies are floors with zero exact pins, and what is installed sits far above them -- fastapi 0.136.3 against a 0.110 floor, cryptography 49.0.0 against 42. No evidence file names a single package version; provenance records the git commit and the schema head and nothing about the code that executed | [`GOAL_REPRODUCIBILITY_2026-08-13.md`](GOAL_REPRODUCIBILITY_2026-08-13.md) opened |
| 2026-08-13 | Does that threaten the method or only the paperwork? | The method. Measure, change one thing, measure again, attribute the difference -- that assumes everything else held still, and SQLAlchemy compiles the queries, DuckDB runs the pipeline and pyarrow owns the memory these gates time. Three attributions were made this session on that assumption without checking it | D2: provenance records the resolved dependency set |
| 2026-08-13 | Why has CI never provisioned a runner? | A billing block, stated by GitHub rather than inferred: every failed run carries the annotation "The job was not started because recent account payments have failed or your spending limit needs to be increased". The repository is private and Actions is enabled, and the failures straddle a monthly quota reset that changed nothing -- which is what a payment block does and exhausted minutes do not. The earlier entry here called it "most likely" minutes or a limit; that was a guess and the annotation was one API call away | C1 blocked on the account owner |
| 2026-08-13 | If C1 cannot be closed, is its intent reachable? | Yes. The literal condition needs a payment method; the intent -- checks that run without being remembered -- does not. The enforcement layer already runs in the suite, and `scripts/hooks/pre-push` runs the seven static checks in 7s on every push with no runner and no minutes. Verified by adding an undeclared verifier and watching the push fail with the ratchet's own message | Opt-in hook; 24 MANUAL checks left to a real runner |
| 2026-08-13 | Has any ratchet in this repository ever actually fired? | Not automatically. GitHub Actions has never completed a run here -- 14 runs since 2026-07-26, every job dead in 2-10s with zero steps, the runner never provisioning. One of the failures is titled "Add automatic production CI gates". Most ratchets do have `test_*.py` wrappers that apply them to the live tree, so the suite covers them; but 11 checks, including the query-bounds ratchet and every PostgreSQL verifier, have no automated execution anywhere and ran this month only because someone typed the command | [`GOAL_2026-08-13.md`](GOAL_2026-08-13.md) opened |
| 2026-08-13 | Why did nothing notice? | Because no check records that it ran. Every gate carries a head, a harness and a capture time; `audit_evidence_corpus.py` prints "Ratchet held" and exits 0 with no timestamp, head, or memory. A gate without provenance cannot be shown stale; a check without provenance cannot be shown to have stopped, so its silence reads exactly like its success | C5/C6: run records and an auditor over them |
| 2026-08-09 | What should the facet threshold bound -- the latency, or the path? | Both, and the path matters more. A latency bound only implies which path ran, and which path ran is what changed silently for three days. `compare` is numeric, so `facet_from_rollup_min: 1` encodes it the way durability encodes `fresh_volume_restores_min`. Verified against the case a latency bound misses: a 12 ms read that did not use the rollup still breaches | 2 thresholds, chosen from 4 observations |
| 2026-08-09 | Would an unmeasured facet have failed the gate? | No -- it would have deleted it. The reading is recorded as `None`, and `compare` raised on `None <= 300`, aborting the run before any evidence was written. An auditor would have seen one fewer gate rather than one more failure. Null now breaches, and the shared comparison has its own tests for the first time | `test_threshold_comparison.py`, 14 assertions |
| 2026-08-09 | Why did the facet gate keep saying `exact` when a rollup existed? | Because nothing could fill it. The table shipped at 0040 and the read path used it whenever populated, but the only caller of `refresh_facet_counts` was a test -- no route, no job, no load hook -- and the reference fixture held 0 rows. B3 was closed on a capability no deployment could reach, and the gate had been telling the truth for three days | `POST /object-sets/facets/refresh`; 2,713.288 -> 9.235 ms |
| 2026-08-09 | Is a fast rollup read the whole story? | No, and recording only it would describe half the price. The read is 9.235 ms because a 2.724 s refresh paid for it, so the gate records both. The aggregate route also silently dropped `max_rollup_age_seconds`, so counts computed once were served forever; the gate now bounds them to 900 s and cannot mistake an abandoned rollup for one it paid for | Both halves recorded; staleness bounded |
| 2026-08-09 | Do all four scale harnesses measure a migrated database? | No, and assuming so is what the new check caught. `benchmark_pipeline_scale.py` overrides `DATABASE_URL` to a throwaway SQLite file before importing the app -- its subject is DuckDB snapshot execution -- so there is no `alembic_version` to read. It refused and crashed rather than emit a defaulted head. That gate now declares none, with the reason at the call site | 3 gates verify a head, 1 states why it cannot |
| 2026-08-09 | Where does the head check belong -- in each harness, or in one place? | One place. `write_evidence` now takes `observed_head`, refuses a mismatch, and records it in provenance, so the rule is stated once instead of four times that can drift. Collaboration is the interesting case: it never opens a database, so it reads `/health/ready` from both replicas and rejects a disagreement -- which measures what the replicas actually serve, and is a stronger check than a connection | 4 harnesses wired, 1 shared rule |
| 2026-08-09 | What stops a harness from quietly dropping the argument again? | A floor in `audit_evidence_corpus.py`: the count of gates declaring a verified database head may rise, never fall. Without it, deleting `observed_head=` still writes a well-formed file and nothing notices -- the difference between a ratchet and an intention. Verified by removing the field and confirming exit 1 | Floor set at 1, rises as gates re-run |
| 2026-08-09 | Did the three stale gates read the 0031 volume? | Almost certainly not -- they drive the app in-process against `DATABASE_URL`, they measure the 10M/50M/200k fixture the dev database demonstrably held at `0041` that day, and the stale volume's `base/` has had no file created since Jul 31. But their files cannot show it, because none of the four benchmark harnesses records the database head either. `/health/ready` already returns `database_head` and `runtime_head` | Answer is inference, not evidence |
| 2026-08-09 | Why did the emitter overwrite real evidence with a failure? | Because I ran it to inspect the journal, and it had no way to be used that way. A recorded FAIL at the current head is sticky by design -- a gate that passes only after repeated attempts has not passed -- so a diagnostic run leaves a failure that outlives its reason. Restored the file from git and gave `aggregate` an `--output-dir` | Inspection no longer mutates the release set |
