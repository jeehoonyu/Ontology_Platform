...
| 2026-08-09 | Did the durability rehearsals measure the schema they claimed? | No. Both stamped `0041` from `current_head()`, which reads the repository's migration files and says what the *code* declares -- while the fixture they measured sat at `0031`, verified structurally: no `geo_min_lat` (0039), no `object_facet_counts` (0040). The gate read PASS on a schema the run never touched, in the gate whose subject is whether a schema survives a restore. Both scripts already read `alembic_version` and neither passed it on | `record()` requires a verified `observed_head`; 2 rows excluded |
| 2026-08-09 | Is that hole specific to durability? | No. None of the ontology-scale, mixed-workload, pipeline-scale or collaboration harnesses reads `alembic_version` either; all four stamp the repository head. Whether each is actually misattributed depends on the database it ran against, but the check is absent in every one | Reported, not silently widened |
