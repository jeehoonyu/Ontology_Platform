...
| --- | --- | --- | --- |
| `components/canvas/PipelineCanvas.tsx` | a node around the pipeline canvas, a selection rectangle over it, an edge from one node's output port to another's input, and receives palette drops | the node's position had no non-drag control at all and now moves from the keyboard; the canvas offers `Add <type>` when it is empty; what a lasso selects, Shift+click and `Select all` select without a drag; `Connect` joins two selected nodes without a port drag | a pipeline node moves with the keyboard |
...
| --- | --- |
| `components/canvas/PipelineCanvas.tsx` | `lasso:canvas`, `node:`, `pipeline-canvas`, `port-in:`, `port-out:` |
