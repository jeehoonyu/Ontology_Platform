...
| **R6** | Authorization coverage is measured, ratcheted, and falling | `unauthorized_mutating_ceiling` at 0 | 42 modules with zero `require_permission`; no ratchet existed | **Open** — `unauthorized_mutating_ceiling` (auth-coverage) 282 -> **24**. 38 routers mounted, `main`'s 23 gated per-route. The four added last were never considered: `ROUTER_PERMISSIONS.get(name, [])` mounted them with no gate at all |
