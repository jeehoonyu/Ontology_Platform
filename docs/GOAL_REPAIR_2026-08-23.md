...
| **R6** | Authorization coverage is measured, ratcheted, and falling | `unauthorized_mutating_ceiling` at 0 | 42 modules with zero `require_permission`; no ratchet existed | **Open** — `unauthorized_mutating_ceiling` (auth-coverage) 282 -> **71**. 34 routers mounted, `main`'s 23 gated per-route. Eight deferred as per-route, one excluded on purpose. The last four were not earned here: the ceiling stood at 75 while the measurement had been 71 since before this branch, and nobody re-recorded it |
