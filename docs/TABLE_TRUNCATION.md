...
**2 of 4** truncations that reach a table render no true count. 86 table elements
...
| `workspaces/ObjectExplorer.tsx` | 160 | `ObjectExplorer` | `query.columns` (slice) | held as columns | **no** |
| `workspaces/OpsWorkspace.tsx` | 38 | `CommandTab` | `events` (slice) | mapped | **no** |
| `workspaces/PipelineBuilder.tsx` | 635 | `OntologyContractPanel` | `issues` (slice) | passed as rows | yes |
...
| `workspaces/OpsWorkspace.tsx` | 49 | `IncidentsTab` | `alerts` (slice) | mapped | no |
| `workspaces/Vertex.tsx` | 295 | `Vertex` | `seedObjects.value` (slice) | mapped | no |
| `workspaces/VisualBuilder.tsx` | 569 | `VisualBuilder` | `collaborators.data.participants` (slice) | mapped | no |
| `workspaces/VisualBuilder.tsx` | 659 | `VisualBuilder` | `preview.sample_output` (slice) | mapped | no |
